...
function Index() {
	...
	function RenderProduct() {
        if(user){
			if (product.length > 0) {
				return product.map((product, index1) => {
					return (JSON.parse(product.image)).map((image, index2) => {
						return (
									...
									<div className="single-products">
										...
										<div className="product-overlay">
											<div className="overlay-content">
												...
												<p>{product.name}</p>
												<a className="btn btn-default add-to-cart"><i className="fa fa-shopping-cart"></i>Add to cart</a>
												<Link to={"/productdetails/" + product.id} style={{marginLeft:'10px'}} className="btn btn-default add-to-cart">More</Link>
											</div>
										</div>
									</div>
									<div className="choose">
										<ul className="nav nav-pills nav-justified">
											<li onClick={() => AddWishList(product.id)}><a ><i className="fa fa-plus-square"></i>Add to wishlist</a></li>
